...
if (isset($_POST['register-submit'])){
    ...
    $count = numberOfEnrolledStudents($enrolled);
    
    if($count >= 30){
        header("Location: ../pages/main.php?error=CourseIsFull");
        exit();
    }

    if(userIsEnrolled($user_email, $enrolled)){
        header("Location: ../pages/main.php?error=userAlreadyEnrolled");
        exit();
    }

    $enrolled = $enrolled . $user_email . ",";
    $sql = "UPDATE courses
            SET enrolled=?
            WHERE course_code =?;";
